...
This module notifies the user if the vehicle moves more than a preset distance(default is 2m) in any direction from the 
   "parked" point.
...
#            [('verbose', bool, False),('versee', bool, True),])
    ...
    def cmd_park(self, args):
        ...
        if len(args) == 0:
            print("Usage: park <status|on|off|radius>\n On: enable departure warning.\n Radius defines the distance in meters (3D) from parked position, when exceeded an alarm is raised.")
        elif args[0] == "status":
            self.status()
        elif args[0] == "on":
            self.park_on()
    ...
    def status(self):
        ...
        if self.parked == True:        
            print("Parked at lat %(lat)f lon=%(lon)f alt=%(alt).1fm max radius=%(max).1fm" %
                   {"lat": self.latp,
                    "lon": self.lonp,
                    "alt": self.altp,
                    "max": self.radius,
    ...
    def park_on(self):
        ...
        self.altp = self.alt
        self.parked=True
        print("Parked at lat %(lat)f lon=%(lon)f alt=%(alt).1fm , you will be notified ft it moves > %(max).1fm" %
               {"lat": self.latp,
                "lon": self.lonp,
                "alt": self.altp,
                "max": self.radius,
    ...
    def idle_task(self):
        ...
        now = time.time()
        if self.parked == True and now-self.last_check > self.check_interval:
            self.last_check = now
            self.dist = self.haversine(self.lon, self.lat, self.alt, self.lonp, self.latp, self.altp)
            if self.dist > self.radius and now-self.last_notification > self.notify_interval:
